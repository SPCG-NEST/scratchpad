...
use logger::CounterCompressedAccount;
...
async fn test() {
    ...
    let compressed_accounts = test_indexer.get_compressed_accounts_by_owner(&logger::ID);
    ...
    R: RpcConnection + MerkleTreeExt,
{
    ...
    let instruction_data = logger::instruction::Create {
        inputs: Vec::new(),
    ...
    let cpi_signer = find_cpi_signer(&logger::ID);

    let accounts = logger::accounts::Create {
        signer: payer.pubkey(),
    ...
    R: RpcConnection + MerkleTreeExt,
{
    ...
    let instruction_data = logger::instruction::Increment {
        inputs,
        proof: rpc_result.proof,
        merkle_context,
    ...
    let accounts = logger::accounts::Increment {
        signer: payer.pubkey(),
        light_system_program: PROGRAM_ID_LIGHT_SYSTEM,
        account_compression_program: PROGRAM_ID_ACCOUNT_COMPRESSION,
        account_compression_authority,
    ...
    R: RpcConnection + MerkleTreeExt,
{
    ...
    let instruction_data = logger::instruction::Delete {
        inputs,
        proof: rpc_result.proof,
        merkle_context,
        merkle_tree_root_index: rpc_result.root_indices[0],
    ...
    let accounts = logger::accounts::Delete {
        signer: payer.pubkey(),
        light_system_program: PROGRAM_ID_LIGHT_SYSTEM,
        account_compression_program: PROGRAM_ID_ACCOUNT_COMPRESSION,
        account_compression_authority,
